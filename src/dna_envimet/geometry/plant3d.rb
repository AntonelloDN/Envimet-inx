  ...
  module EnvimetInx
    module Geometry
      class Plant3d
	    ...
	    DEFAULT_PLANT3D_MATERIAL = "0000C2"
        ...
        def initialize(name = " ", others = {})
		  ...
		  values = {
            material:DEFAULT_PLANT3D_MATERIAL
          }
	    ...
	    def self.add_plants(plant)
	      @@objects << plant if plant.is_a?(Plant3d) && !plant.guid.nil?
	    end
		...
		def self.set_index(objects)
		  index_list = objects.grep(Geometry::Plant3d).map {|obj| obj.index}
